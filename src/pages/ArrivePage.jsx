import { useState } from 'react';
import { DAYS, DAY_DATA, PRACTICE_DETAIL, STATES, getCurrentSeason } from '../data/practices';
import { saveStateCheckIn, getTodayState } from '../data/storage';
import { getDailyQuote, FIVE_ROOTS } from '../data/teachings';
import Timer from '../components/Timer';

const STATE_TO_PRACTICE = {
  activated: 'threshold',
  flat: 'walk',
  pain: 'rest',
  foggy: 'threshold',
  grounded: null,
  tender: 'zazen',
};

export default function ArrivePage({ onNavigate, settings }) {
  const now = new Date();
  const dayName = DAYS[now.getDay()];
  const today = DAY_DATA[dayName];
  const hour = now.getHours();
  const season = getCurrentSeason();
  const savedState = getTodayState();
  const [selectedState, setSelectedState] = useState(
    savedState ? STATES.find((s) => s.id === savedState) : null
  );
  const [showTimer, setShowTimer] = useState(false);

  const quote = getDailyQuote(settings?.activeTeachers);
  const root = quote.root ? FIVE_ROOTS.find(r => r.id === quote.root) : null;

  const timeOfDay = hour < 12 ? 'morning' : hour < 17 ? 'afternoon' : 'evening';
  const greeting = timeOfDay === 'morning'
    ? 'Good morning'
    : timeOfDay === 'afternoon'
      ? 'Good afternoon'
      : 'Good evening';

  const suggestedPractice = (() => {
    if (selectedState) {
      const override = STATE_TO_PRACTICE[selectedState.id];
      if (override) return PRACTICE_DETAIL[override];
    }
    if (timeOfDay === 'morning') return PRACTICE_DETAIL.zazen;
    const eveningPractice = today.practices.find(
      (p) => p !== 'zazen' && p !== 'rest'
    );
    return eveningPractice
      ? PRACTICE_DETAIL[eveningPractice]
      : PRACTICE_DETAIL.threshold;
  })();

  const handleStateSelect = (state) => {
    if (selectedState?.id === state.id) {
      setSelectedState(null);
    } else {
      setSelectedState(state);
      saveStateCheckIn(state.id);
    }
  };

  const isZazen = suggestedPractice.label === 'Zazen';

  return (
    <div className="px-6 pt-10">
      {/* Header */}
      <div className="mb-8">
        <h1
          className="font-display font-light tracking-wide mb-1"
          style={{ fontSize: '30px', lineHeight: 1.15 }}
        >
          {greeting}
        </h1>
        <p className="font-display italic font-light" style={{ fontSize: '15px', color: 'var(--c-text-faint)' }}>
          {dayName} — {today.theme}
        </p>
        <p
          className="font-body mt-1.5"
          style={{ fontSize: '11px', color: 'var(--c-text-hint)', letterSpacing: '0.08em' }}
        >
          {season.name} · {season.description}
        </p>
      </div>

      {/* Daily wisdom */}
      <div className="mb-10 px-2">
        <p
          className="font-display italic font-light leading-relaxed"
          style={{ fontSize: '17px', color: 'var(--c-text-muted)', lineHeight: 1.55 }}
        >
          "{quote.text}"
        </p>
        <div className="flex items-center gap-2 mt-2">
          <span
            className="font-body not-italic"
            style={{ fontSize: '11px', color: 'var(--c-text-hint)', letterSpacing: '0.06em' }}
          >
            — {quote.author}
          </span>
          {root && (
            <span
              className="font-body px-2 py-0.5 rounded-full"
              style={{
                fontSize: '9px',
                letterSpacing: '0.1em',
                textTransform: 'uppercase',
                color: root.color,
                background: `${root.color}12`,
                border: `1px solid ${root.color}20`,
              }}
            >
              {root.glyph} {root.name.split(' ')[0]}
            </span>
          )}
        </div>
      </div>

      {/* State check-in */}
      <div className="mb-8">
        <h3 className="section-label">how are you arriving?</h3>
        <div className="card">
          <div className="flex flex-wrap gap-1.5">
            {STATES.map((s) => {
              const active = selectedState?.id === s.id;
              return (
                <button
                  key={s.id}
                  onClick={() => handleStateSelect(s)}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-full font-body transition-all"
                  style={{
                    fontSize: '12px',
                    border: active
                      ? `1px solid ${s.color}`
                      : '1px solid var(--c-border)',
                    background: active ? `${s.color}15` : 'transparent',
                    color: active ? s.color : 'var(--c-text-faint)',
                  }}
                >
                  <span>{s.emoji}</span> {s.label}
                </button>
              );
            })}
          </div>
          {selectedState && (
            <div
              className="mt-4 p-3.5 rounded-[10px]"
              style={{
                background: `${selectedState.color}08`,
                border: `1px solid ${selectedState.color}15`,
              }}
            >
              <p
                className="font-body italic leading-relaxed"
                style={{ fontSize: '13px', color: selectedState.color }}
              >
                {selectedState.suggestion}
              </p>
            </div>
          )}
        </div>
      </div>

      {/* Suggested practice */}
      <div className="mb-8">
        <h3 className="section-label">
          {selectedState ? 'for this moment' : 'suggested now'}
        </h3>
        <div
          className="rounded-[14px] p-5"
          style={{
            background: 'var(--c-surface-active)',
            border: `1px solid ${suggestedPractice.color}25`,
            boxShadow: `0 2px 20px ${suggestedPractice.color}10`,
          }}
        >
          <div className="flex items-baseline gap-3 mb-2">
            <span className="text-2xl leading-none" style={{ color: suggestedPractice.color }}>
              {suggestedPractice.icon}
            </span>
            <div>
              <div className="font-display font-normal" style={{ fontSize: '18px' }}>
                {suggestedPractice.label}
              </div>
              <div className="font-body" style={{ fontSize: '12px', color: 'var(--c-text-faint)' }}>
                {suggestedPractice.duration}
              </div>
            </div>
          </div>
          <p
            className="font-body leading-relaxed mb-4"
            style={{ fontSize: '13.5px', color: 'var(--c-text-muted)', lineHeight: 1.6 }}
          >
            {suggestedPractice.description}
          </p>
          <div className="flex gap-2 flex-wrap">
            {isZazen && (
              <button
                onClick={() => setShowTimer(!showTimer)}
                className="px-5 py-3 rounded-full font-body tracking-wide"
                style={{
                  fontSize: '13px',
                  fontWeight: 500,
                  color: 'var(--c-bg)',
                  background: `linear-gradient(135deg, ${suggestedPractice.color}, ${suggestedPractice.color}dd)`,
                }}
              >
                {showTimer ? 'Close timer' : 'Begin timer'}
              </button>
            )}
            <button
              onClick={() => onNavigate?.('earth')}
              className="px-4 py-2.5 rounded-full font-body"
              style={{
                fontSize: '12px',
                color: 'var(--season-accent)',
                border: '1px solid var(--c-border)',
                background: 'var(--c-surface)',
              }}
            >
              Nature prompt
            </button>
            <button
              onClick={() => onNavigate?.('spiral')}
              className="px-4 py-2.5 rounded-full font-body"
              style={{
                fontSize: '12px',
                color: 'var(--c-text-faint)',
                border: '1px solid var(--c-border)',
                background: 'var(--c-surface)',
              }}
            >
              The Spiral
            </button>
          </div>
        </div>
        {showTimer && isZazen && (
          <div className="card mt-3">
            <Timer />
          </div>
        )}
      </div>

      {/* Today's rhythm */}
      <div className="mb-4">
        <h3 className="section-label">today's rhythm</h3>
        <div className="card">
          <div className="flex flex-wrap gap-1.5 mb-3">
            {today.practices.map((p) => {
              const d = PRACTICE_DETAIL[p];
              return (
                <span
                  key={p}
                  className="inline-block font-body px-3 py-1 rounded-full"
                  style={{
                    fontSize: '11px',
                    background: `${d.color}15`,
                    color: d.color,
                    border: `1px solid ${d.color}25`,
                  }}
                >
                  {d.icon} {d.label}
                </span>
              );
            })}
          </div>
          <p
            className="font-body italic leading-relaxed"
            style={{ fontSize: '13px', color: 'var(--c-text-faint)', lineHeight: 1.6 }}
          >
            {today.note}
          </p>
        </div>
      </div>
    </div>
  );
}
